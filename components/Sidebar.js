...
export default function Sidebar({ isExpanded = false, setIsExpanded }) {
  ...
  return (
    <aside
      className={`fixed left-6 top-[47%] -translate-y-1/2 h-fit min-h-[580px] rounded-[2rem] bg-white/40 backdrop-blur-xl border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.05)] flex flex-col py-6 z-50 transition-all duration-300 ${isExpanded ? 'w-56 px-5 items-start' : 'w-20 px-2 items-center'
        }`}
    >
      <div className={`mb-6 flex items-center transition-all duration-300 ${isExpanded ? 'w-full px-4 gap-3 justify-start' : 'w-16 h-16 justify-center'}`}>
        <img
      ...
      <nav className={`flex flex-col gap-3 flex-1 w-full ${isExpanded ? 'items-stretch' : 'items-center'}`}>
        ...
        <div className={`mt-auto border-t border-slate-200/50 pt-5 flex flex-col gap-4 w-full ${isExpanded ? 'items-start' : 'items-center'}`}>
          <div className={`flex items-center gap-4 w-full p-3 text-slate-400 hover:text-indigo-400 transition-colors cursor-pointer rounded-2xl ${!isExpanded ? 'justify-center w-12 h-12' : 'px-4'
            }`}>
          ...
          <div className={`flex items-center gap-4 w-full p-3 opacity-60 ${!isExpanded ? 'flex-col gap-1 justify-center' : 'px-4'
            }`}>
          ...
          <button
            ...
            className={`p-2 rounded-full hover:bg-white/50 text-slate-400 hover:text-indigo-500 transition-all cursor-pointer flex items-center justify-center mx-auto ${!isExpanded ? 'w-10 h-10' : 'w-10 h-10 ml-auto mr-0'
              }`}
          >
